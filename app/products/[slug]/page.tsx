...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${product.name} · ${product.slug} | StackMug`,
    description: product.description.slice(0, 155),
    openGraph: {
      title: `${product.name} | StackMug`,
      description: product.description.slice(0, 155),
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-6">
            ...
            <div className="mt-10 rounded-xl bg-white border border-slate-200 p-6 shadow-xs">
              <h3 className="text-xs font-bold text-slate-900 font-mono tracking-wider uppercase">
                System Specs
              </h3>
            ...
            <div className="mt-8 border border-slate-200 bg-white rounded-xl p-4 shadow-xs">
              ...
              <p className="text-[11px] text-slate-500 leading-relaxed mt-1">
                Notice the{' '}
                <code className="bg-slate-100 px-1 py-0.5 rounded text-[10px] text-slate-800 border border-slate-200/50">
                  generateMetadata
                </code>{' '}
                export in this page. During the upcoming modules, we will inject a schema JSON-LD
                block right below to help Google match your product parameters inside Search Result
